function [Hq,tq,hq,Dq,Fq,qRegLine] = MFDFA1(signal,scale,q,m,Fig,eps)
...
% eps:          noise level to remove from the RMS calculation
%
% OUTPUT VARIABLES---------------------------------------------------------
%
...
for ns=1:length(scale),
    ...
    % Remove RMS values below the noise level.
    RMS_scale{ns}(RMS_scale{ns} < eps) = [];
    for nq=1:length(q),
