function dataout = load_gt3x_Actigraph(file)
% [dataout] = load_gt9x_Actigraph(file)
% inputs  - file, this input is not used.
% outputs - dataout, this output is not used
% Remarks
% - This function is a placeholder so gt9x files can be moved and copied
%   within the DAMVi App. These files are binary and currently there is not a
%   way to load them directly into MATLAB.
% Future Work
% - Possibly these files could be read directly into MATLAB.
% Aug 2022 - Created by Ben Senderling, bsender@bu.edu
%% Begin Code

dataout = [];

end
